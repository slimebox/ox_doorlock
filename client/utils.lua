...
local function pickLock(entity)
	...
	if not door or PickingLock or not door.lockpick or (not Config.CanPickUnlockedDoors and door.state == 0) then return false end
...
exports('pickClosestDoor', function()
	...
	return pickLock(ClosestDoor.entity)
end)
...
function lock(entity)
	local door = getDoorFromEntity(entity)
	TriggerServerEvent('ox_doorlock:setState', door.id, 1)
end
...
CreateThread(function()
	...
	if target.ox then
		...
		target.exp:addGlobalObject({
			{
				name = "Unlock Door",
				label = locale('unlock-door'),
				icon = 'fas fa-unlock-keyhole',
				onSelect = unlock,
				canInteract = canUnlock,
				anyItem = true,
				distance = 3
			}
		})
